...
class TelegramCollector:
    ...
    async def collect(
        ...
        **kwargs: Any,
        ...
        if not items:
            self.logger.warning("Telegram Bot API returned no matching messages. Ensure the bot is added to channels and has received messages.")
        else:
            self.logger.info(f"Collected {len(items)} real items from Telegram")
